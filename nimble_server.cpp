...
//#include "nimble/ble_store_config.h"
...
// C-style callback implementations
extern "C" {
...
void print_addr(const void* addr) {
    const uint8_t* u8p;
    u8p = reinterpret_cast<const uint8_t*>(addr);  // Explicit cast here

    // Now you can use u8p as a const uint8_t* pointer
    // For example, print the address or access the data it points to
    for (int i = 0; i < 6; i++) {  // Assuming addr points to a 6-byte array (e.g., a MAC address)
        printf("%02X ", u8p[i]);
    }
    printf("\n");
}
...
void startble()
{
    ...
    rc = ble_svc_gap_device_name_set(tag);
    assert(rc == 0);
...
void NIMBLEServer::ble_write(uint16_t conn_handle, uint16_t attr_handle, const uint8_t *data, size_t len) 
{
    struct os_mbuf *om;
    int rc;

    // Create an mbuf from the data
    om = ble_hs_mbuf_from_flat(data, len);
    if (om == nullptr) {
        ESP_LOGE("BLE", "Failed to allocate mbuf for write");
        return;
    }

    // Perform the write operation
    rc = ble_gattc_write_flat(conn_handle, attr_handle, data, len, nullptr, nullptr);
    if (rc != 0) {
        ESP_LOGE("BLE", "Failed to write to characteristic, conn_handle=%d, attr_handle=%d, rc=%d", conn_handle, attr_handle, rc);
    } else {
        ESP_LOGI("BLE", "Successfully wrote to characteristic, conn_handle=%d, attr_handle=%d", conn_handle, attr_handle);
    }
}


void NIMBLEServer::setup() {
    instance_ = this;
    ESP_LOGD("nimble_server", "Initializing BLE...");
    startble();
...
void NIMBLEServer::start_advertising_() {
}

void NIMBLEServer::loop() {
    // Empty implementation - no periodic tasks needed
}
